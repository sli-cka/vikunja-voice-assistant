...
from custom_components.vikunja.helpers.detailed_response_formatter import friendly_due_phrase

def test_friendly_due_phrase_today():
    today = datetime.now().strftime('%Y-%m-%d')
    assert friendly_due_phrase(today) in {today, 'today'}  # may map to today or keep raw if time absent
